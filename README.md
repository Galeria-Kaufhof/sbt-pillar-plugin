...
This sbt plugin allows to run Cassandra schema/data migrations from sbt (using [the GALERIA Kaufhof pillar fork](https://github.com/Galeria-Kaufhof/pillar)).
...
```
addSbtPlugin("io.ino" %% "sbt-pillar-plugin" % "2.2.0")
```
...
<dt>createMigrationsTable</dt><dd>Creates pillar's <code>applied_migrations</code> table without creating the keyspace beforehand</dd>
